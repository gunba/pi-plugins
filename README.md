...
- `pi-browser` — minimal browser/PWA control surface for Pi sessions, with multi-tab worker streaming, session-file browsing, mobile voice dictation support, RPC extension UI dialogs, and a `host_screenshot` tool for agent-requested desktop screenshots.
- `pi-web-search` — single-provider Perplexity Sonar web search plus guarded HTTPS page fetch, with untrusted-content wrappers, prompt-injection risk flags, public-network URL checks, and browser-like request headers.
