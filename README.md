...
- `pi-browser` — minimal browser/PWA control surface for Pi sessions, with multi-tab worker streaming, cached session-file browsing, incremental desktop-session refresh, lazy transcript rendering, mobile voice dictation support, RPC extension UI dialogs, and a `host_screenshot` tool for agent-requested desktop screenshots.
- `pi-file-links` — turns project-relative paths, absolute Linux paths, tilde paths, Windows paths, UNC paths, and existing paths with spaces into clickable terminal file links while stripping generated links before model context.
...
- `pi-scheduler` — adds `/schedule <delay> <message>` and an agent-facing `schedule` tool for delayed messages (`15m`, `5h`, `5.5h`, `30d`) with a compact queued-message panel, countdowns, `ctrl+o` expansion, and cancel/list command reminders.
