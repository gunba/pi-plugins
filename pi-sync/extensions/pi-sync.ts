...
const MANAGED_GITIGNORE_LINES = [
	...
	"agent/bin/fd",
	"",
	"# Local credentials and auth material",
	"agent/auth.json",
	"agent/local-env.json",
	"agent/mcp-oauth/",
	"web-search.json",
	"",
	"# OS and editor noise",
...
async function piUpdateExtensions(pi: ExtensionAPI): Promise<ExecResult> {
	return pi.exec("pi", ["update", "--extensions"], {
...
async function ensureRepo(pi: ExtensionAPI, root: string): Promise<void> {
	...
	throw new Error(
		`Pi setup is not a git repo yet. Run /pi-sync init <private-repo-url> first.\nRoot: ${root}`,
	);
}
...
async function remoteTrackedFiles(
	pi: ExtensionAPI,
	root: string,
	ref: string,
): Promise<string[]> {
...
async function commitLocalChanges(
	...
	if (diff.code !== 1)
		throw new Error(
			`git diff --cached --quiet failed${execOutput(diff) ? `\n${execOutput(diff)}` : ""}`,
		);
...
async function pushBranch(pi: ExtensionAPI, root: string): Promise<void> {
	...
	if (!origin)
		throw new Error(
			"No origin remote configured. Run /pi-sync remote <private-repo-url> first.",
		);
...
async function refreshPackages(
	...
	notify(
		ctx,
		`Pi setup synced, but pi update --extensions failed${output ? `:\n${output}` : "."}`,
		"warning",
	);
}
...
async function showStatus(
	...
	if (!isRepo(root)) {
		notify(
			ctx,
			`Pi setup sync is not initialized.\nRoot: ${root}\nRun /pi-sync init <private-repo-url>.`,
			"warning",
		);
		return;
	}
	...
	const statusText =
		status.code === 0
			...
			: execOutput(status) || "status unavailable";
	notify(
		ctx,
...
async function handlePull(
	...
	notify(
		ctx,
		"Pi setup pulled. Run /reload or restart Pi to load changed resources.",
	);
}
...
async function handlePush(
	...
	await ensureRepo(pi, root);
	const changedIgnore = ensureManagedGitignore(root);
	const committed = await commitLocalChanges(
		pi,
		root,
	...
	notify(
		ctx,
		`Pi setup pushed.${committed ? "" : " No local changes to commit."}${changedIgnore && !committed ? " Managed .gitignore refreshed." : ""}`,
	);
}
...
async function handleSync(
	...
	notify(
		ctx,
		`Pi setup synced.${committed ? "" : " No local changes to commit."} Run /reload or restart Pi to load changed resources.`,
	);
}
...
async function handleBootstrap(
	...
	if (await hasHead(pi, root)) {
		...
		notify(
			ctx,
			"Pi setup bootstrap complete. Existing repo fast-forwarded; run /reload or restart Pi.",
		);
		return;
	}
	...
	notify(
		ctx,
		`Pi setup bootstrap complete.${backupDir ? ` Previous local files backed up to ${backupDir}.` : ""} Run /reload or restart Pi.`,
	);
}
	...
	switch (action) {
		...
		case "ignore":
			...
			notify(
				ctx,
				ensureManagedGitignore(root)
					? "Pi setup .gitignore refreshed."
					: "Pi setup .gitignore already current.",
			);
			return;
		case "help":
		case "--help":
