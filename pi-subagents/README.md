...
The view is **on by default and persisted**. `/subagents` toggles it and saves the choice. `/subagent <name>` opens that agent's transcript in a modal editor and then offers a follow-up message prompt; `/subagent <name> <message>` sends/resumes directly; `/subagent kill <name|*>` hard-stops from the UI. The panel appears whenever the run has subagents, hides when the run is empty, and clears on the next user message after every subagent is terminal (`done`, `error`, or `stopped`) and its messages have been consumed.
