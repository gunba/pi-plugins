...
- `wait()` — yield until a subagent needs you (a question or approval request) or one finishes. Always interruptible (abort signal / Esc), so control always returns.

The usual loop: `spawn` a team, call `wait` to yield while they run, answer questions with `message` (you keep your full tools to investigate first), then `wait` again. A subagent's final answer arrives as a message from it; deliverables are files it writes.
...
- **Self-cleaning and resumable.** A subagent runs only while working: on completion it pushes its result to its parent and exits, so there are no idle processes. Re-addressing a finished agent with `message` resumes its session — identity and history persist, compute does not. An atomic `.active` lock stops two callers resuming the same agent at once.
...
- **Stuck agents never brick the session.** No agent-chosen timeouts. A watchdog notices a direct child with no progress for an interval and raises a UI prompt to stop it; stopping makes the node terminal and resolves any `wait` blocked on it. The human is the circuit-breaker.
