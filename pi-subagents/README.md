...
- **Stuck agents never brick the session.** No agent-chosen timeouts. A watchdog notices a direct child with no progress for an interval and raises a UI prompt to stop it; stopping makes the node terminal and posts a repair event to the parent. `wait` also refuses to block when there are no active children or pending messages. The human is the circuit-breaker.
