...
const SETTINGS_FILE =
	process.env.PI_MEMEDIT_SETTINGS ||
	join(os.homedir(), ".pi", "agent", "memedit", "settings.json");
const DEFAULT_SETTINGS: MemeditSettings = {
	enabled: true,
	liveEnabled: true,
...
const AGENT_SESSION_PATCH_KEY = Symbol.for(
	"pi.memedit.agentSessionPatchInstalled",
);
...
function stateSyncSessions(): Set<string> {
	const globalState = globalThis as typeof globalThis & {
		[AGENT_STATE_SYNC_KEY]?: Set<string>;
	};
...
function markAgentStateNeedsSync(sessionId: unknown): void {
	if (typeof sessionId === "string" && sessionId)
		stateSyncSessions().add(sessionId);
}

function installAgentSessionPatch(): void {
	const globalState = globalThis as typeof globalThis & {
		[AGENT_SESSION_PATCH_KEY]?: boolean;
	};
	if (globalState[AGENT_SESSION_PATCH_KEY]) return;

	const proto = (AgentSession as unknown as { prototype?: AnyRecord })
		.prototype;
	...
	proto._checkCompaction = async function patchedCheckCompaction(
		...
		if (
			...
			stateSyncSessions().delete(sessionId)
		) {
			const sessionContext = this.sessionManager?.buildSessionContext?.();
			if (Array.isArray(sessionContext?.messages) && this.agent?.state) {
				this.agent.state.messages = sessionContext.messages.filter(
					(message: AnyRecord) => !isStatusAgentMessage(message),
				);
			}
		}
...
function loadSettings(): MemeditSettings {
	try {
		const parsed = JSON.parse(
			readFileSync(SETTINGS_FILE, "utf8"),
		) as Partial<MemeditSettings>;
		settingsLoadError = undefined;
		return {
			enabled: readBoolean(parsed.enabled, DEFAULT_SETTINGS.enabled),
			liveEnabled: readBoolean(
				parsed.liveEnabled,
				DEFAULT_SETTINGS.liveEnabled,
			),
			showDeletedItems: readBoolean(
				parsed.showDeletedItems,
				DEFAULT_SETTINGS.showDeletedItems,
			),
		};
	} catch (error) {
		...
		return {
			enabled: false,
			liveEnabled: false,
			showDeletedItems: DEFAULT_SETTINGS.showDeletedItems,
		};
	}
}
...
function resolveInitialEnabled(persistedEnabled: boolean): boolean {
	...
	if (
		isDisabled(process.env.PI_MEMEDIT) ||
		isDisabled(process.env.PI_MEMEDIT_ENABLED)
	)
		return false;
	if (
		isEnabled(process.env.PI_MEMEDIT) ||
		isEnabled(process.env.PI_MEMEDIT_ENABLED)
	)
		return true;
...
function currentTurnEntryIds(branch: SessionEntry[]): Set<string> {
	...
	for (let i = branch.length - 1; i >= 0; i--) {
		...
		if (
			entry.type === "message" &&
			(entry.message as AnyRecord)?.role === "assistant"
		) {
			start = i;
			break;
		}
...
function protectFinalAssistantTextResponse(
	branch: SessionEntry[],
	scopedEntryIds: Set<string>,
): Set<string> {
...
function collectContextItems(
	...
	const append = (entry: SessionEntry) => {
		...
		items.push({
			entry,
			scoped,
			removable: entryIsRemovable(entry, scopedEntryIds, protectedEntryIds),
		});
	};
...
function entryPromptRole(entry: SessionEntry): string {
	if (entry.type === "message") {
		...
		if (message.role === "toolResult")
			return `toolResult:${message.toolName ?? "tool"}`;
		return String(message.role ?? "message");
	}
	if (entry.type === "custom_message")
		return `custom:${entry.customType ?? "message"}`;
...
function bashExecutionText(message: AnyRecord | undefined): string {
	...
	if (typeof message.exitCode === "number")
		lines.push(`[exitCode]\n${message.exitCode}`);
	if (typeof message.signal === "string" && message.signal.trim())
		lines.push(`[signal]\n${message.signal}`);
	return lines.join("\n\n");
}
...
function entryPromptText(entry: SessionEntry): string {
	if (entry.type === "message")
		return messagePromptText(entry.message as AnyRecord);
...
function buildPruneMessages(
	items: ContextItem[],
	options: { upcomingRequest?: string; promptMode?: PrunePromptMode } = {},
): Message[] {
	...
	return message.content
		.filter(
			(part): part is { type: "text"; text: string } =>
				part.type === "text" && typeof (part as AnyRecord).text === "string",
		)
		.map((part) => part.text)
		.join("\n")
		.trim();
...
function hasAnthropicOAuthSystemMarker(block: unknown): boolean {
	...
	return (
		...
		block.text.includes(ANTHROPIC_BILLING_HEADER_PREFIX)
	);
}
...
function isAnthropicOAuthPayload(payload: AnthropicPayload): boolean {
	return (
		...
		payload.system.some(hasAnthropicOAuthSystemMarker)
	);
}
...
function shapeAnthropicOAuthPayload(payload: unknown): unknown {
	if (!isAnthropicMessagesPayload(payload) || !isAnthropicOAuthPayload(payload))
		return payload;
...
function shapeMemeditProviderPayload(
	payload: unknown,
	apiKey: string,
): unknown {
	return isAnthropicOAuthApiKey(apiKey)
		? shapeAnthropicOAuthPayload(payload)
		: payload;
}
...
function usesOpenAiTokenizer(model: AnyRecord | undefined): boolean {
	...
	if (
		/anthropic|claude|google|gemini|mistral|bedrock/.test(`${provider} ${id}`)
	)
		return false;
	return /openai|azure|codex|gpt-|\bo[1-9]|chatgpt/.test(`${provider} ${id}`);
}

function openAiEncoding(
	model: AnyRecord | undefined,
): ReturnType<typeof getEncoding> | undefined {
	if (!usesOpenAiTokenizer(model)) return undefined;
	const modelId = String(model?.id ?? "gpt-4o");
	const encodingName = /gpt-4o|gpt-4\.1|gpt-5|\bo[1-9]|codex/i.test(modelId)
		? "o200k_base"
		: "cl100k_base";
...
function estimateMessageTokens(message: AnyRecord, model?: AnyRecord): number {
	if (usesOpenAiTokenizer(model)) {
		if (
			...
			return estimateContentTokens(message.content, model);
		if (message.role === "assistant") {
			return (message.content ?? []).reduce(
				(total: number, block: AnyRecord) => {
					if (block?.type === "text" && typeof block.text === "string")
						return total + estimateTextTokens(block.text, model);
					if (block?.type === "thinking" && typeof block.thinking === "string")
						return total + estimateTextTokens(block.thinking, model);
					if (block?.type === "toolCall")
						return (
							total +
							estimateTextTokens(
								`${block.name ?? ""}${safeJson(block.arguments ?? {})}`,
								model,
							)
						);
					return total;
		...
		if (message.role === "bashExecution")
			return estimateTextTokens(bashExecutionText(message), model);
		if (
			message.role === "branchSummary" ||
			message.role === "compactionSummary"
		)
			return estimateTextTokens(String(message.summary ?? ""), model);
	}
...
function estimateEntriesTokens(
	...
	return entries.reduce(
		(total, entry) => total + estimateEntryTokens(entry, model),
		0,
	);
}

function estimateContextItemsTokens(
	items: ContextItem[],
	model?: AnyRecord,
): number {
	return items.reduce(
		(total, item) => total + estimateEntryTokens(item.entry, model),
		0,
	);
}
...
function previewEntry(entry: SessionEntry): DeletedItem {
	if (entry.type === "message") {
		...
		return {
			id: entry.id,
			role,
			text: truncateText(entryPromptText(entry) || "(no text)"),
		};
	}
	if (entry.type === "custom_message") {
		return {
			...
			text: truncateText(contentText(entry.content) || "(no text)"),
		};
	}
	if (entry.type === "branch_summary") {
		return {
			id: entry.id,
			role: "branchSummary",
			text: truncateText(String(entry.summary ?? "")),
		};
	}
	return {
		id: entry.id,
		role: entry.type,
		text: truncateText(JSON.stringify(entry)),
	};
}
...
function parseDeleteNumbers(text: string): number[] {
	...
	try {
		...
		throw new Error("pi-memedit received invalid JSON from the pruning model", {
			cause: error,
		});
	}
	...
	for (const value of raw) {
		const candidate =
			typeof value === "number" || typeof value === "string"
				? value
				: (value?.id ?? value?.number ?? value?.message);
...
function expandToolDependencies(
	entries: SessionEntry[],
	initialDeleteIds: Set<string>,
): Set<string> {
	...
	for (const entry of entries) {
		...
		if (message?.role === "assistant") {
			...
			for (const callId of callIds)
				assistantEntryByToolCall.set(callId, entry.id);
		} else if (
			message?.role === "toolResult" &&
			typeof message.toolCallId === "string"
		) {
			const resultEntries =
				resultEntriesByToolCall.get(message.toolCallId) ?? new Set<string>();
	...
	while (changed) {
		...
		for (const entry of entries) {
			...
			if (message?.role === "assistant") {
				...
				const assistantEntryId = assistantEntryByToolCall.get(
					message.toolCallId,
				);
...
function replacementFirstKept(
	compaction: SessionEntry,
	keptIds: Set<string>,
	byId: Map<string, SessionEntry>,
): string | undefined {
	if (!compaction.firstKeptEntryId || keptIds.has(compaction.firstKeptEntryId))
...
function computeCacheImpact(
	...
	const tokenCounts = recacheScope.map((entry) =>
		estimateEntryTokens(entry, model),
	);
	const scopeTokens = tokenCounts.reduce((total, value) => total + value, 0);
	const firstDeletedIndex = recacheScope.findIndex((entry) =>
		expandedDeleteIds.has(entry.id),
	);
	...
	for (let i = 0; i < recacheScope.length; i++) {
		...
		if (i < firstDeletedIndex) {
			...
			if (expandedDeleteIds.has(recacheScope[i].id))
				droppedTailTokens += tokens;
...
function projectHardDelete(
	...
	if (expandedDeleteIds.size === 0) {
		const recacheScope =
			recacheScopeEntries && recacheScopeEntries.length > 0
				? recacheScopeEntries
				: entries.filter(entryParticipatesInContext);
		const cacheImpact = computeCacheImpact(
			recacheScope,
			expandedDeleteIds,
			model,
		);
	...
	const removed = entries.filter(
		(entry) =>
			expandedDeleteIds.has(entry.id) ||
			(entry.type === "label" && expandedDeleteIds.has(entry.targetId)),
	);
	const countedEntries = removed.filter(
		(entry) => entryParticipatesInContext(entry) && !uncountedIds.has(entry.id),
	);
...
function assertSessionRewriteSupported(manager: AnyRecord): void {
	...
	if (missing.length > 0)
		throw new Error(
			`Pi session manager does not support safe pi-memedit rewrite; missing ${missing.join(", ")}`,
		);
}

function validateRewrittenReferences(
	entries: SessionEntry[],
	keptIds: Set<string>,
): void {
	for (const entry of entries) {
		if (entry.parentId && !keptIds.has(entry.parentId))
			throw new Error(
				`pi-memedit rewrite left dangling parentId on ${entry.id}`,
			);
		if (
			entry.type === "compaction" &&
			entry.firstKeptEntryId &&
			!keptIds.has(entry.firstKeptEntryId)
		) {
			throw new Error(
				`pi-memedit rewrite left dangling firstKeptEntryId on ${entry.id}`,
			);
		}
		if (
			entry.type === "branch_summary" &&
			entry.fromId &&
			!keptIds.has(entry.fromId)
		) {
			throw new Error(
				`pi-memedit rewrite left dangling branch_summary fromId on ${entry.id}`,
			);
		}
		if (
			entry.type === "label" &&
			typeof entry.targetId === "string" &&
			!keptIds.has(entry.targetId)
		) {
			throw new Error(
				`pi-memedit rewrite left dangling label targetId on ${entry.id}`,
			);
		}
	}
}
...
function applyHardDelete(
	...
	if (!header)
		throw new Error(
			"Current session has no header; cannot rewrite session log",
		);

	const entries = manager.getEntries() as SessionEntry[];
	const projection = projectHardDelete(
		entries,
		deleteIds,
		uncountedIds,
		recacheScopeEntries,
	...
	if (typeof sessionFile !== "string" || !sessionFile)
		throw new Error(
			"Current session has no writable session file; cannot rewrite session log",
		);
	...
	const retainedEntries = entries.filter((entry) => {
		...
		if (
			entry.type === "label" &&
			projection.expandedDeleteIds.has(entry.targetId)
		)
			return false;
		return true;
	});
	const retainedIds = new Set(retainedEntries.map((entry) => entry.id));
	const keptEntries = retainedEntries.filter(
		(entry) =>
			entry.type !== "label" ||
			typeof entry.targetId !== "string" ||
			retainedIds.has(entry.targetId),
	);
	...
	const rewrittenEntries = keptEntries.map((entry) => {
		...
		if (next.type === "compaction") {
			...
			if (replacement && keptIds.has(replacement))
				next.firstKeptEntryId = replacement;
			else delete next.firstKeptEntryId;
		}
		if (
			next.type === "branch_summary" &&
			next.fromId &&
			!keptIds.has(next.fromId)
		) {
	...
	writeUtf8FileAtomic(
		sessionFile,
		`${nextFileEntries.map((entry) => JSON.stringify(entry)).join("\n")}\n`,
	);

	manager.fileEntries = nextFileEntries;
	manager._buildIndex();
	manager.leafId =
		oldLeafId && keptIds.has(oldLeafId)
			? oldLeafId
			: nearestKeptAncestor(oldLeafId, keptIds, byId);
...
function isStatusAgentMessage(message: AnyRecord): boolean {
	return (
		message?.role === "custom" && message.customType === STATUS_MESSAGE_TYPE
	);
}
...
function currentSessionMessages(
	...
	return Array.isArray(sessionContext?.messages)
		? sessionContext.messages.filter(
				(message: AnyRecord) => !isStatusAgentMessage(message),
			)
		: undefined;
}
	...
	return new Set(
		entries
			.filter(
				(entry) =>
					entry.type === "custom_message" &&
					entry.customType === STATUS_MESSAGE_TYPE,
			)
			.map((entry) => entry.id),
	);
}
...
function isLikelyContextOverflowMessage(message: string | undefined): boolean {
	return /context (window|length|size|limit)|too many tokens|maximum (context|tokens)|prompt too long|input too large|exceeds? .*context/i.test(
		message ?? "",
	);
}
...
function recacheEconomics(
	...
	recacheCost: number;
	savingPerCallCost: number;
	breakEvenCalls: number | undefined;
} {
	const pricing = cachePricing(model);
	const recachePenaltyPerToken = Math.max(
		0,
		pricing.cacheWritePerToken - pricing.cacheReadPerToken,
	);
	...
	const breakEvenCalls =
		savingPerCallCost > 0
			? Math.ceil(recacheCost / savingPerCallCost)
			: undefined;
...
function estimateForwardHorizon(
	ctx: ExtensionContext,
	branch: SessionEntry[],
	...
	const contextNow =
		typeof usage?.tokens === "number" ? usage.tokens : fallbackContextTokens;
	...
	return Math.max(
		1,
		Math.min(MEMEDIT_HORIZON_CALLS, Math.round(callsToCompaction)),
	);
}

function pruneNetValue(
	cacheImpact: CacheImpact,
	model: AnyRecord | undefined,
	horizonCalls: number,
): number {
	const { recacheCost, savingPerCallCost } = recacheEconomics(
		cacheImpact.droppedTailTokens,
		cacheImpact.keptTailTokens,
		model,
	);
...
function selectProfitableDeletions(
	...
	const pricing = cachePricing(model);
	const penaltyPerToken = Math.max(
		0,
		pricing.cacheWritePerToken - pricing.cacheReadPerToken,
	);
	...
	if (readPerToken <= 0 && penaltyPerToken <= 0)
		return { appliedIds: new Set(expanded), declined: 0 };
	...
	for (let i = recacheScope.length - 1; i >= 0; i--) {
		...
		const net =
			readPerToken * horizonCalls * droppedSuffix -
			penaltyPerToken * keptSuffix;
		if (net > bestNet) {
			bestNet = net;
			bestIndex = i;
		}
	...
	if (bestIndex >= 0) {
		for (let i = bestIndex; i < recacheScope.length; i++)
			if (isDeleted[i]) appliedIds.add(recacheScope[i].id);
	}
	const declined = [...selectedIds].reduce(
		(count, id) => (appliedIds.has(id) ? count : count + 1),
		0,
	);
...
function formatTokens(value: number | undefined): string {
	...
	if (Math.abs(count) < 1_000_000)
		return `${(count / 1000).toFixed(Math.abs(count) < 10_000 ? 1 : 0)}k`;
...
function cumulativeContextPercent(): number {
	return telemetry.contextTokensBefore > 0
		? (telemetry.tokensSaved / telemetry.contextTokensBefore) * 100
		: 0;
}

function cumulativeBreakEvenCalls(): number | undefined {
	return telemetry.savingPerCallCost > 0
		? Math.ceil(telemetry.recacheCost / telemetry.savingPerCallCost)
		: undefined;
}

function contextUsageFields(
	ctx: ExtensionContext,
): Pick<
	MemeditStats,
	| "contextWindowTokensBefore"
	| "contextWindowTokensLimit"
	| "contextWindowPercentBefore"
> {
	const usage = ctx.getContextUsage();
	return {
		contextWindowTokensBefore:
...
function contextWindowSuffix(stats: MemeditStats): string {
	...
	const tokenDetails =
		typeof stats.contextWindowTokensBefore === "number" &&
		typeof stats.contextWindowTokensLimit === "number"
...
function styledStatusSummary(stats: MemeditStats, theme: Theme): string[] {
	...
	if (stats.error)
		lines.push(
			`${theme.fg("warning", "Reason")} ${theme.fg("muted", truncateText(stats.error, 220))}`,
		);
	return lines;
}
...
function renderStatusMessage(
	...
	const lines = [
		styledStatusHeadline(stats, theme),
		...styledStatusSummary(stats, theme),
	];
	if (expanded) {
		lines.push(
		...
		if (typeof stats.contextWindowPercentBefore === "number") {
			const tokenDetails =
				...
				typeof stats.contextWindowTokensLimit === "number"
					...
					: "";
			lines.push(
				`${theme.fg("dim", "Model window before prune")} ${theme.fg("muted", `${formatPercent(stats.contextWindowPercentBefore)}${tokenDetails}`)}`,
			);
		}
		...
		if (
			showDeletedItems &&
			stats.deletedItems &&
			stats.deletedItems.length > 0
		) {
			lines.push(
				theme.fg("warning", "Removed"),
				...stats.deletedItems.map((item) =>
					theme.fg("muted", formatDeletedItem(item)),
				),
			);
		}
	} else {
	...
	return (_tui: unknown, theme: Theme) => {
		...
		box.addChild(
			new Text(
				theme.fg(
					"dim",
					"Memory edit in progress — Pi will continue automatically.",
				),
				0,
				0,
			),
...
function showPruningUi(ctx: ExtensionContext, candidates: number): void {
	...
	ctx.ui.setStatus(
		SYSTEM_STATUS_KEY,
		ctx.ui.theme.fg("warning", `memedit:pruning(${candidates})`),
	);
	if (ctx.mode === "tui")
...
function clearPruningUi(ctx: ExtensionContext): void {
	if (ctx.hasUI && ctx.mode === "tui")
		ctx.ui.setWidget(PRUNING_WIDGET_KEY, undefined);
}

function footerStatusText(): string {
	if (!enabled) return "";
	if (realizedSavingsCost > 0)
		return `memedit:${formatCost(realizedSavingsCost)} saved`;
	if (lastStats?.status === "applied")
		return `memedit:${formatTokens(lastStats.tokensSaved)} pruned`;
...
function memeditStatusText(stats: MemeditStats): string {
	if (stats.status === "applied")
		return `${EXTENSION_NAME}: deleted ${stats.deleted} context entr${stats.deleted === 1 ? "y" : "ies"}.`;
	if (stats.status === "noop")
		return `${EXTENSION_NAME}: no context entries deleted.`;
	if (stats.status === "skipped")
		return `${EXTENSION_NAME}: skipped${stats.error ? ` (${stats.error})` : ""}.`;
	return `${EXTENSION_NAME}: failed${stats.error ? ` (${stats.error})` : ""}.`;
}

function statusMessage(stats: MemeditStats) {
	const lines = [
		`${memeditStatusText(stats)} Candidates: ${stats.candidates}; selected: ${stats.selected}; ignored: ${stats.ignored}.`,
	];
	if (
		stats.status === "applied" ||
		stats.status === "noop" ||
...
function preflightAllowsRun(
	...
	if (mode === "live") {
		return (
			...
			(candidates.length >= LIVE_MIN_CANDIDATES &&
				candidateTokens >= Math.floor(LIVE_MIN_CANDIDATE_TOKENS / 2))
		);
	}

	return (
		candidateTokens >= AUTO_MIN_CANDIDATE_TOKENS ||
		(candidates.length >= AUTO_MIN_CANDIDATES &&
			candidateTokens >= Math.floor(AUTO_MIN_CANDIDATE_TOKENS / 2))
	);
}
...
async function runMemedit(
	...
	const protectedEntryIds = protectFinalAssistantTextResponse(
		branch,
		scopedEntryIds,
	);
	for (const id of options.extraProtectedEntryIds ?? [])
		protectedEntryIds.add(id);
	const items = collectContextItems(branch, scopedEntryIds, protectedEntryIds);
	const promptItems =
		options.promptScope === "scoped"
			? items.filter((item) => item.scoped)
			: items;
	const activeContextEntries = items.map((item) => item.entry);
	const contextTokensBefore = estimateContextItemsTokens(
		items,
		model as AnyRecord,
	);
	const candidates = items.filter(
		(item) => item.removable && item.number !== undefined,
	);
	if (candidates.length === 0) {
		...
		return mode === "manual" || options.showStatusMessage
			? lastStats
			: undefined;
	}

	if (!preflightAllowsRun(candidates, model as AnyRecord, mode)) {
		const candidateTokens = estimateContextItemsTokens(
			candidates,
			model as AnyRecord,
		);
		lastStats = {
			...
			contextPercentSaved:
				contextTokensBefore > 0
					? (candidateTokens / contextTokensBefore) * 100
					: 0,
	...
	try {
		...
		if (!auth.ok || !auth.apiKey) {
			const authError =
				auth.ok === false ? auth.error : `No API key for ${model.provider}`;
			lastStats = {
				at: Date.now(),
				mode,
				status: "skipped",
		...
		const response = await completeSimple(
			model,
			{
				...
				messages: buildPruneMessages(promptItems, {
		...
		if (response.stopReason === "error" || response.stopReason === "aborted") {
			throw new Error(
				response.errorMessage ||
					`memedit prune request ended with ${response.stopReason}`,
			);
		}

		const deleteNumbers = parseDeleteNumbers(assistantText(response));
		const itemByNumber = new Map(
			candidates.map((item) => [item.number!, item]),
		);
		...
		const pruneTokens = usageTotalTokens(
			response.usage as AnyRecord | undefined,
		);
		...
		const horizonCalls = estimateForwardHorizon(
			ctx,
			branch,
			contextTokensBefore,
		);
		const entries = (manager.getEntries?.() ?? []) as SessionEntry[];
		const selection = selectProfitableDeletions(
		...
		if (appliedSelectedIds.size > 0) {
			const projected = projectHardDelete(
				entries,
				new Set([...appliedSelectedIds, ...housekeepingIds]),
				housekeepingIds,
				activeContextEntries,
				model as AnyRecord,
			);
		...
		const contextPercentSaved =
			contextTokensBefore > 0
				? (deleted.tokensSaved / contextTokensBefore) * 100
				: 0;
		lastStats = {
			at: Date.now(),
			mode,
		...
		lastStats = {
			...
			...contextUsageBefore,
			error: overflow
				? "prune request exceeded context; left unchanged for normal Pi compaction"
				: message,
		};
		if (ctx.hasUI && !overflow)
			ctx.ui.notify(`pi-memedit failed: ${lastStats.error}`, "warning");
	} finally {
		running = false;
		clearPruningUi(ctx);
		if (ctx.hasUI) ctx.ui.setStatus(SYSTEM_STATUS_KEY, footerStatusText());
	...
	for (let index = messages.length - 1; index >= 0; index--) {
		...
		if (
			...
			return candidate.errorMessage
				? `${candidate.stopReason}: ${candidate.errorMessage}`
				: String(candidate.stopReason);
		}
		return undefined;
	}
	return undefined;
...
function formatStats(): string {
	...
	if (settingsLoadError)
		settingsLines.push(
			`Settings warning: ${settingsLoadError}; pi-memedit failed closed.`,
		);
	if (!lastStats)
		return [...settingsLines, "pi-memedit has not run in this session."].join(
			"\n",
		);
	const lines = [
		...settingsLines,
	...
	if (typeof lastStats.contextWindowPercentBefore === "number") {
		const tokenDetails =
			typeof lastStats.contextWindowTokensBefore === "number" &&
			typeof lastStats.contextWindowTokensLimit === "number"
				? ` (${formatTokens(lastStats.contextWindowTokensBefore)} / ${formatTokens(lastStats.contextWindowTokensLimit)})`
				: "";
		lines.push(
			`Model window before last prune: ${formatPercent(lastStats.contextWindowPercentBefore)}${tokenDetails}`,
		);
	}
	if (
		showDeletedItems &&
		lastStats.deletedItems &&
		lastStats.deletedItems.length > 0
	) {
...
export default function memedit(pi: ExtensionAPI) {
	pi.registerMessageRenderer<MemeditStats>(
		STATUS_MESSAGE_TYPE,
		(message, { expanded }, theme) => {
			...
			if (
				!stats ||
				typeof stats !== "object" ||
				typeof (stats as MemeditStats).status !== "string"
			)
				return undefined;
			return renderStatusMessage(
				stats as MemeditStats,
				expanded === true,
				theme,
			);
		},
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const branch = ((ctx.sessionManager as AnyRecord).getBranch?.() ??
			[]) as SessionEntry[];
		...
		if (ctx.hasUI) {
			...
			if (settingsLoadError)
				ctx.ui.notify(
					`pi-memedit failed closed: ${settingsLoadError}`,
					"warning",
				);
		}
	});
	...
	pi.on("context", async (event, ctx) => {
		...
		if (filtered.length !== event.messages.length)
			return { messages: filtered as never };
	});
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		const hasToolCalls = Array.isArray(assistantMessage?.content)
			? assistantMessage.content.some(
					(part: AnyRecord) => part?.type === "toolCall",
				)
			: false;
		if (
		...
		if (
			lastLivePruneTurnIndex >= 0 &&
			event.turnIndex - lastLivePruneTurnIndex < 2
		)
			return;
		...
		if (stats?.status === "applied") {
			...
			if (ctx.hasUI)
				ctx.ui.notify(
					`pi-memedit live pruned ${formatTokens(stats.tokensSaved)}; ${breakEvenText(stats)}`,
					"info",
				);
		}
	});
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		if (unsuccessfulReason) {
			...
			lastStats = skippedStats(
				`agent run did not complete cleanly (${unsuccessfulReason})`,
			);
			if (ctx.hasUI)
				ctx.ui.setStatus(SYSTEM_STATUS_KEY, enabled ? "memedit:skipped" : "");
			return;
		}
		if (ctx.hasUI) ctx.ui.setStatus(SYSTEM_STATUS_KEY, footerStatusText());
	});
	...
	pi.registerCommand("memedit", {
		description:
			"Show or control automatic post-turn context/session-log memory editing",
		handler: async (args, ctx) => {
			...
			if (command === "on" || command === "enable") {
				...
				if (ctx.hasUI) {
					...
					ctx.ui.notify(
						`pi-memedit enabled and persisted to ${SETTINGS_FILE}`,
						"info",
					);
				}
				return;
			...
			if (command === "off" || command === "disable") {
				...
				if (ctx.hasUI) {
					...
					ctx.ui.notify(
						`pi-memedit disabled and persisted to ${SETTINGS_FILE}`,
						"info",
					);
				}
				return;
			...
			if (liveMatch) {
				...
				if (ctx.hasUI) {
					ctx.ui.setStatus(SYSTEM_STATUS_KEY, footerStatusText());
					ctx.ui.notify(
						`pi-memedit live pruning ${next ? "enabled" : "disabled"} and persisted to ${SETTINGS_FILE}`,
						"info",
					);
				}
				return;
			}
			const showMatch = command.match(
				/^(?:show|show-deleted|details|verbose|output|removed|removed-text)\s+(on|off|enable|disable)$/,
			);
			if (showMatch) {
				...
				if (ctx.hasUI)
					ctx.ui.notify(
						`pi-memedit removed-text output ${next ? "enabled" : "disabled"} and persisted to ${SETTINGS_FILE}`,
						"info",
					);
				return;
			}
