...
Only requests through this session's registered `openai-codex` provider are covered. Independently launched tools or SDK sessions also need the plugin loaded and active. The saved default applies where those sessions share the same agent directory. Proxy environment settings apply to WebSockets; SSE continues through Pi's supplied fetch implementation.
